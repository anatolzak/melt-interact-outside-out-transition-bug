...
<section>
	...
	{#if $open && asChild}
		<slot {content} />
	{:else if $open}
		<div transition:scale={{ duration: 300, start: 0.8 }} use:melt={$content}>
...
<style lang="postcss">
	section {
		@apply inline-flex flex-col gap-3;
	}
	...
	[data-melt-calendar-cell] {
		@apply flex h-6 w-6 cursor-pointer select-none items-center justify-center rounded-lg p-4 hover:bg-magnum-200  focus:ring focus:ring-magnum-400;
	}
	...
	[data-melt-calendar-cell][data-selected] {
		@apply bg-magnum-300 text-magnum-800;
	}
